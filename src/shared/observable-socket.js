import {Observable} from 'rxjs';

export class ObservableSocket {
  get isConnected() {
    return this._state.isConnected;
  }

  get isConnecting() {
    return this._state.isReconnecting;
  }

  get isTotallyDead() {
    return !this._state.isConnected && !this._state.isReconnecting;
  }

  constructor(socket) {
    this._socket = socket;
    this._state = {};

    this.status$ = Observable.merge(
      this.on$('connect').map(() => ({isConnected: true})),
      this.on$('disconnect').map(() => ({isConnecting: false})),
      this.on$('reconnecting').map(attempt => ({isConnected: false, isConnecting: true, attempt})),
      this.on$('reconnect_failed').map(attempt => ({isConnected: false, isConnecting: false})))
      .publishReplay()
      .refCount();

    this.status$.subscribe(state => this._state = state);
  }

  on$(event) {
    return Observable.fromEvent(this._socket, event);
  }

  on(event, callback) {
    this._socket.on(event, callback);
  }

  off(event, callback) {
    this._socket.off(event, callback);
  }

  emit(event, arg) {
    this._socket.emit(event, arg);
  }
}